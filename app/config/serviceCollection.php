<?php
    ...
    [
        ...
        'entity-manager' => function($setting = []) {
            ...
            return $em->setQueryBuilder(\chilimatic\lib\di\DIFactory::getInstance()->get('query-builder'));
        },
        'query-builder' => function($setting = []) {
            $queryBuilder = new \chilimatic\lib\database\orm\MysqlQueryBuilder();
            return $queryBuilder->setCache(\chilimatic\lib\di\DIFactory::getInstance()->get('cache', ['type' => 'shmop']));
        },
